    ...
    <div className="flex items-center mb-4">
      <div className='flex justify-between items-center gap-20'>
        <div className='flex items-center justify-between'>
          <Icon className="text-2xl text-[#FF5722] dark:text-gray-200 mr-4" />
          <h3 className="text-lg dark:text-slate-50 font-semibold text-light-text">{title}</h3>
        </div>
        <p className="text-[12px] w-[80px] text-[#FF5722] bg-white dark:text-slate-50 flex dark:bg-[#30302f] p-1 rounded">{pricingType}</p>
      </div>
    </div>
    ...
    <ul className="mb-6">
      {features.map((feature, index) => (
        <li key={index} className="flex items-center mb-2 text-light-text dark:text-gray-200">
          <BsCheck className="text-[#FF5722] dark:text-gray-200 mr-2" />
          {feature}
        </li>
      ))}
    </ul>
    <hr className='dark:border-gray-700 dark:border-solid mb-4' />
    <Link to="/contact" className="flex items-center justify-center w-full bg-[#FF5722] dark:bg-dark-body dark:border dark:border-gray-700 dark:border-solid text-white px-4 py-2 rounded-lg hover:translate-y-[-3px] duration-300">
      {buttonLabel}
    </Link>
  </div>
);
    ...
    <div className="bg-white dot-pattern dark:bg-dark-body transition-colors py-10 px-4">
      <div className="w-full md:w-4/5 mx-auto">
        ...
        <div className="flex items-center justify-between flex-col md:flex-row bg-light-body dark:bg-[#1B1B1A] px-6 py-6 rounded-lg w-full mt-6 border dark:border-gray-800 dark:border-solid font-inter" >
          <div className='flex gap-2 items-center'>
            <div>
              <div className='flex items-center gap-2'>
                <TbRainbow size={20} className='text-[#FF5722] dark:text-white' />
          ...
          <Link to="/contact" className='bg-[#FF5722] dark:border flex w-full items-center justify-center md:w-1/6 dark:border-gray-700 dark:border-solid dark:bg-dark-body text-white px-4 py-2 rounded-lg hover:translate-y-[-3px] duration-300' data-aos="fade-up" data-aos-delay="200">
